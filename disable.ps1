##################################################
# HelloID-Conn-Prov-Target-Triaspect-Triasweb-Disable
# PowerShell V2
##################################################
    ...
    switch ($action) {
        'DisableAccount' {
            $splatDisableUserParams = @{
                Uri         = "$($actionContext.Configuration.BaseUrl)/api/users/close-account?method=id&value=$($actionContext.References.Account)"
                Method      = 'PUT'
                Certificate = $certificate
                Headers     = $headers
            }
