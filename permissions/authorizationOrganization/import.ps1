...
try {
    ...
    # Retrieve all unique authorizationOrganizationCodes from accounts property in accounts.
    $importedPermissions = @{}
    foreach ($importedAccount in $importedAccounts) {
        foreach ($authorizedOrganizationCode in $importedAccount.authorizedOrganizationCodes) {
            if (![string]::IsNullOrWhiteSpace($authorizedOrganizationCode)) {
                if (-not $importedPermissions.ContainsKey($authorizedOrganizationCode)) {
                    $importedPermissions[$authorizedOrganizationCode] = @()
                }
                $importedPermissions[$authorizedOrganizationCode] += $importedAccount.id
            }
        }
    }
